		...
		return (
			...
			>
				<motion.div className="flex justify-center sm:justify-between items-center w-full">
					<motion.div className={`day-number plusJakartaSans800 ${props.day.freeDay ? "bg-[#44D375]/20 text-[#1fd15e]" : "bg-LightColor text-SecondColor"}`}>
						{props.day.day}
					</motion.div>
					<motion.div
				...
				{props.day.events.length != 0 && (
					<div className="flex absolute left-1/2 -translate-x-1/2 2xs:bottom-1 md:hidden xs:bottom-2 -bottom-1.5 items-center gap-x-1">
						...
						<p className={`hidden sm:block text-xs plusJakartaSans800`}>{props.day.events.length}</p>
					</div>
				)}
			</motion.div>
		);
