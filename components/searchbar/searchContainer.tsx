	...
	return (
		<>
			<motion.div
				...
				animate={{ opacity: 1 }}
				exit={{ opacity: 0 }}
				className="flex flex-col gap-0.5 md:gap-2 fixed z-40 max-w-screen-xs sm:max-w-screen-sm top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full md:max-w-screen-md lg:max-w-screen-xl"
			>
				...
				{input != "" && (results.posts.length > 0 || results.events.length > 0) && (
					<ResultsContainer toggle={props.toggle} fetched={fetched} events={results.events} posts={results.posts} searchPhrase={input} />
				)}
			</motion.div>
			...
			<motion.div
				...
				animate={{ opacity: 1 }}
				exit={{ opacity: 0 }}
				className="fixed z-20 cursor-pointer top-0 backdrop-blur-sm left-0 w-screen h-screen"
