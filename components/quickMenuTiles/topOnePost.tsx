...
export default function TopOnePost() {
	...
	if (topOnePost != undefined)
		return (
			...
			>
				<div
					className={`bg-white z-10 text-MainDarkGray absolute top-4 xs:top-8 left-4 xs:left-8 w-fit text-xs sm:text-base 2xl:text-xl rounded-2xl py-0.75 px-4 sm:px-8 plusJakartaSansFont800`}
				>
				...
				<div className="bg-MainDarkGray/80 flex flex-col text-white 2xs:group-hover:-translate-y-full duration-300 ease-out transition-all -translate-y-10">
					<h3
						className={`from-MainDarkGray/0 line-clamp-2 via-MainDarkGray/50 bg-gradient-to-b 3xl:text-4xl md:text-xl 4xl:text-5xl to-MainDarkGray/80 pt-4 w-full text-base xs:text-2xl lg:text-3xl absolute 2xs:-translate-y-full -translate-y-2/3 xs:px-6 md:px-8 left-0 px-3 poppinsFont700`}
					>
						{topOnePost.title}
					</h3>
